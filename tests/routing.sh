...
grep -F -- 'release=v1.0.0' "$ROOT/handbook.md" >/dev/null ||
  fix_wave_fail 'handbook does not select v1.0.0'
grep -F 'VERSION=v1.0.0' "$ROOT/PACKAGE-DESIGN.md" >/dev/null ||
  fix_wave_fail 'package design does not select v1.0.0'
