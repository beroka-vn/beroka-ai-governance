...
repository Git root, the compact team command reuses healthy client-owned
GitHub OAuth or starts `gh` browser OAuth only when missing, then delegates
connector/OAuth setup to the verified release. The selected AI client and `gh`
are one-time workstation prerequisites:
...
After tag and commit verification, interactive launcher setup offers to install
missing `jq` through `apt-get`, `dnf`, or `brew`. Declining returns
...
An existing registration changes its pin only when the same compact command
includes explicit `--upgrade`:
...
CLI bootstrap snapshots managed files before delegating to transactional
`update`, so the final repository PR decision includes lock changes. `update`
rejects older targets; `rollback` is the only downgrade path.
