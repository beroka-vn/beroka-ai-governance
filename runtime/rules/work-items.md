...
- Provider status follows `To Do -> In Progress` when accepted work starts and
  ...
  assignee, runs a fresh `jira-write` preflight, and transitions the item to
  `In Review`; an item already in `In Review` is idempotently complete. It then
  resolves related documentation only by exact Confluence content ID. If the
