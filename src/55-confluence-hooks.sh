...
CONFLUENCE_STOP_ROOTS
  ...
  if [ "$dh_event" = pre ]; then
    ...
    printf '%s\n' "$dh_args" | jq -e '
      ...
      (.parentId|id) and (.spaceId|id) and
    ...
    ' >/dev/null 2>&1 || die CLIENT_BODY_GATE_REQUIRED 'Use one Markdown body and explicit numeric spaceId/parentId; unsupported fields or formats require a reviewed adapter'
    ...
    CURSOR_WORKSPACE=$dh_repo CURSOR_TOOL_INPUT=$dh_args CURSOR_TOOL_TEXT=$dh_action
